...
(deftest c-1-neg
  ...
  (let [db (atom {})]
    (with-context db {:year 2017 :person "Alice"}
      {:gross-income 33200
       :basic-standard-deduction 2000
       :additional-standard-deduction 3000
    ...
    (let [standard-deduction
          (query db :standard-deduction {:person "Alice" :year 2017})]
...
(deftest c-2-a-i-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-c-2-a-i
          (query db :section-c-2-a-i {:person "Alice" :year 2017})]
...
(deftest c-2-a-ii-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-c-2-a-ii
          (query db :section-c-2-a-ii {:person "Alice" :year 2017})]
...
(deftest c-2-b-neg
  ...
  (let [db (atom {})]
    ...
    (let [basic-standard-deduction
          (query db :basic-standard-deduction {:person "Alice" :year 2017})]
...
(deftest c-3-neg
  ...
  (let [db (atom {})]
    ...
    (let [additional-standard-deduction
          (query db :additional-standard-deduction
...
(deftest c-5-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-c-5 (query db :section-c-5 {:person "Bob" :year 2017})]
      (is (not section-c-5)))))
...
(deftest c-5-pos
  ...
  (let [db (atom {})]
    ...
    (let [basic-standard-deduction
          (query db :basic-standard-deduction {:person "Bob" :year 2017})]
...
(deftest c-6-a-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-c-6-a (query db :section-c-6-a {:person "Alice" :year 2017})]
  ...
  (let [db (atom {})]
    (with-context db {:year 2017 :person "Alice"}
      {:gross-income 33200
       :married true
       :spouse "Bob"
       :nonresident-alien false})
    (with-context db {:year 2017 :person "Bob"}
      {:married true
       :spouse "Alice"})
    ;; Section 63(c)(6)(B) applies to Alice for 2017. Contradiction
    (let [section-c-6-b (query db :section-c-6-b {:person "Alice" :year 2017})]
...
(deftest c-6-d-neg
  ...
  (let [db (atom {})]
    (with-context db {:year 2021 :person "Walter Brown Family Trust II"}
      {:estate-or-trust false
       ;; the rest of this information is not explicit in the text
       :common-trust-fund false
       :partnership false})
    ;; Section 63(c)(6)(D) applies to the Walter Brown Family Trust II for 2021. Contradiction
    (let [section-c-6-d (query db :section-c-6-d
                               {:person "Walter Brown Family Trust II"
                                :year 2021})]
...
(deftest c-7-i-neg
  ...
  (let [db (atom {})]
    ...
    (let [basic-standard-deduction
          (query db :basic-standard-deduction {:person "Alice" :year 2019})]
      (is (not= basic-standard-deduction 4400)))))
...
(deftest f-1-a-neg
  ...
  (let [db (atom {})]
    ...
    (with-context db {:year 2017 :person "Bob"}
      {:married true
       :spouse "Alice"})
    (with-key db :birth-date
      {{:person "Alice"} [1950 3 2]
       {:person "Bob"} [1955 3 3]})
    ;; Section 63(f)(1)(A) applies to Bob in 2017. Contradiction
    (let [section-f-1-a (query db :section-f-1-a {:person "Bob" :year 2017})]
...
(deftest f-1-a-pos
  ...
  (let [db (atom {})]
    ...
    (let [section-f-1-a (query db :section-f-1-a {:person "Alice" :year 2017})]
      (is section-f-1-a))))
  ...
  (let [db (atom {})]
    (with-context db {:year 2017 :person "Alice"}
      {:gross-income 33200
       :married true
       :spouse "Bob"
       :additional-exemption-for-spouse true})
    (with-context db {:year 2017 :person "Bob"}
      {:married true
       :spouse "Alice"})
    (with-key db :birth-date
    ...
    (let [section-f-1-b (query db :section-f-1-b {:person "Alice" :year 2017})]
...
(deftest f-1-b-pos
  ...
  (let [db (atom {})]
    ...
    (let [section-f-1-b (query db :section-f-1-b {:person "Bob" :year 2017})]
      (is section-f-1-b))))
...
(deftest f-2-a-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-f-2-a (query db :section-f-2-a {:person "Bob" :year 2017})]
...
(deftest f-2-a-pos
  ...
  (let [db (atom {})]
    ...
    (let [section-f-2-a (query db :section-f-2-a {:person "Alice" :year 2017})]
      (is section-f-2-a))))
...
(deftest f-2-b-neg
  ...
  (let [db (atom {})]
    ...
    (let [section-f-2-b (query db :section-f-2-b {:person "Alice" :year 2017})]
...
(deftest f-2-b-pos
  ...
  (let [db (atom {})]
    ...
    (let [section-f-2-b (query db :section-f-2-b {:person "Bob" :year 2017})]
      (is section-f-2-b))))
